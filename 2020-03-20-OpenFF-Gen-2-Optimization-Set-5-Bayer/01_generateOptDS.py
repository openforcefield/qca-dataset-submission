...
def selectDiverseMols(paramDict, parameter, eps=0.5, min_samples=5):
    ...
    for label, mols in mols_by_cluster.items():
        if label != -1:
            ...
            molSmiles=OEMolToSmiles(random.choice(mols))

            #check the cluster and see if there are any molecules that fall bellow 25 heavy atoms
            heavyAtomsCluster=[]
            """
            for m in mols:
                print(m)
                heavyAtomsCluster.append(oechem.OECount(m, oechem.OEIsHeavy()))
            print(heavyAtomsCluster)
            if min(heavyAtomsCluster)<20:
                heavyAtomCount=21
                while heavyAtomCount > 20:
                    randomMolecule=random.choice(mols)

                    molSmiles=OEMolToSmiles(randomMolecule)

                    heavyAtomCount= oechem.OECount(randomMolecule, oechem.OEIsHeavy())
            else:
                randomMolecule=random.choice(mols)
                molSmiles=OEMolToSmiles(randomMolecule)
                heavyAtomCount= oechem.OECount(randomMolecule, oechem.OEIsHeavy())
            """
            #if condition of heavyAtomCount is less than 25 is satisifed in while loop, then append mol
...
def makeJson(smiles):
    ...
    for mol in oemols:
        ...
        states = fragmenter.states.enumerate_states(mol, max_states=5, stereoisomers=True, tautomers=True)
        for s in states:
            ...
            try:
                ...
                conformers = fragmenter.chemi.generate_conformers(m, max_confs=5)
            except RuntimeError:
