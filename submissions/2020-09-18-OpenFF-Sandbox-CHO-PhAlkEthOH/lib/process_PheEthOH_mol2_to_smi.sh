...
unzip -q PhEthOH_mol2files_frosst-20200919T083119Z-001.zip
...
for i in $(ls -1 *.smi | sort -k 1.9n ) ; do
	cat $i
done | tqdm --total=$(ls -1 *.smi | wc -l) --ncols=80 | bzip2 -c > ../../PhEthOH.smi.bz2

cd ../

rm -r PhEthOH_mol2files_frosst

echo "Wrote PhEthOH.smi.bz2"
