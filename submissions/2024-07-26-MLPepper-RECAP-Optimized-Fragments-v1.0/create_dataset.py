...
def main():
    ...
    esp_50k = load_dataset('esp_50k_Br_singlepoint_dataset.json.bz2')
    b_and_si = load_dataset('b_si_singlepoint_dataset.json.bz2')
    ...
    dataset = BasicDataset(
        ...
        description="A dataset of diverse fragments generated by combining Simon's 50k charge dataset and Lily's Br substituted subset along with new fragments "
        "containing B and Si. These were generated using the Recap decomosition scheme from RDKit on the B and Si dataset from SPICE2. Every molecule has had 5 conformers generated using RDKit which were"
        ...
        metadata=Metadata(
            submitter="jthorton",
            long_description_url="https://github.com/openforcefield/qca-dataset-submission/tree/master/submissions/2024-07-26-MLPepper-RECAP-Optimized-Fragments-v1.0"
        ),
        compute_tag="mlpepper"
    )
    ...
    expected_molecules = esp_50k.n_molecules + b_and_si.n_molecules
    expected_records = esp_50k.n_records + b_and_si.n_records
    # track the inchikey to make sure we have unique molecules only
    all_inchikey = set()
    # add each mlecule to the dataset so the metadata is correct
    for index, entry in tqdm.tqdm(esp_50k.dataset.items(), desc="Adding ESP50k", ncols=80):
    ...
    for index, entry in tqdm.tqdm(b_and_si.dataset.items(), desc="Adding B & Si", ncols=80):
        off_mol = entry.get_off_molecule(include_conformers=True)
        inchikey = off_mol.to_inchikey(fixed_hydrogens=True)
        if inchikey in all_inchikey:
            continue
    ...
    print(f"Expected records: {expected_records}, actaul: {total_records}")
