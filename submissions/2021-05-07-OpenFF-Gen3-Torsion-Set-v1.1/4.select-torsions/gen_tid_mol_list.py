...
def gen_canonical_isomeric_smiles(smi):
    
    # molecule = Molecule.from_smiles(smi)
    try:
        molecule = Molecule.from_smiles(smi)
        isomeric_smiles = canonical_isomeric_smiles = molecule.to_smiles(isomeric=True, explicit_hydrogens=True, mapped=True)
    except:
    ...
    torsion_match = molecule.chemical_environment_matches(isomeric_smiles)[0]
    molecule.properties["atom_map"] = {j: i + 1 for i, j in enumerate(torsion_match)}
    center_bond = set(molecule.properties["atom_map"].keys())

    canonical_isomeric_smiles = molecule.to_smiles(isomeric=True, explicit_hydrogens=True, mapped=False)
...
def list_matching_torsions(smi_file, forcefield):
    ...
    for smi in tqdm(list_of_smiles): 
        try: 
            off_mol, mol_index, center_bond = gen_canonical_isomeric_smiles(smi)
...
def filter_torsions_coverage(torsions_coverage, oemol):
    ...
    for torsion_param, indices_list in torsions_coverage.items():
        ...
        if re.search("[^!]#1:1", torsion_param.smirks):
            ...
            heavy_atoms = 3
        elif torsion_param.smirks == '[*:1]-[#8:2]-[#8H1:3]-[*:4]': # hard coded
            heavy_atoms = 3
...
def main():
    ...
    pickle.dump(failed_smi, open('failed_smi.p','wb'))
